...
public class FB2toPDF {
    ...
    private void createHeaderSlot(PdfPTable table, HeaderSlotSettings slotSettings) throws FB2toPDFException {
        ...
        if (slotSettings.enabled) {
            ...
            query += "declare function fb:cut-right($string as xs:string?, $length as xs:integer) { replace(replace($string,concat('^(.{', $length, '}).+$'),'$1Е'), '^(.*)\\W.*Е', '$1Е') }; ";
            query += "declare function fb:cut-left($string as xs:string?, $length as xs:integer) { replace(replace($string,concat('^.+(.{', $length, '})$'),'Е$1'), 'Е\\w*\\W(.*)$', 'Е$1') }; ";
