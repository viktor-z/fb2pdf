...
public class FB2toPDF {
    ...
    private void addFontChangeOutline(Map<Integer, Integer> pageElementMap, Map<Integer, Integer> elementPageMap, int maxPageNumLength) {
        for (int srcpage : pageElementMap.keySet()) {
            ...
            //System.out.println(String.format("srcpage: %s, element: %s, destpage: %s", srcpageLabel, element, destpage));
    ...
    private void createHeaderSlot(PdfPTable table, HeaderSlotSettings slotSettings) throws FB2toPDFException {
        ...
        if (slotSettings.enabled) {
            ...
            queryBuilder.append("declare function fb:cut-right($string as xs:string?, $length as xs:integer) { replace(replace($string,concat('^(.{', $length, '}).+$'),'$1Е'), '^(.*)\\W.*Е', '$1Е') }; "); 
            queryBuilder.append("declare function fb:cut-left($string as xs:string?, $length as xs:integer) { replace(replace($string,concat('^.+(.{', $length, '})$'),'Е$1'), 'Е\\w*\\W(.*)$', 'Е$1') }; "); 
    ...
    private void addFontChangeOutline() {
        System.out.println("Building style change outline...");
