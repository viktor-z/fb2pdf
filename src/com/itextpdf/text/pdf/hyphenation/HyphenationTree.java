...
public class HyphenationTree extends TernaryTree
     ...
     * the hyphenated word or null if word is not hyphenated.
     */
    //public Hyphenation hyphenate(char[] w, int offset, int len, //VIKTORZ --
    public Hyphenation hyphenate(char[] w, int offset, int origlen, //VIKTORZ ++
                                 int remainCharCount, int pushCharCount) {
        int i;

        //char[] word = new char[len + 3]; //VIKTORZ --
        char[] word = new char[origlen + 3]; //VIKTORZ  ++

        // normalize word
        char[] c = new char[2];
        int iIgnoreAtBeginning = 0;
        //int iLength = len; //VIKTORZ --
        int iLength = origlen; //VIKTORZ ++
        boolean bEndOfLetters = false;
        //for (i = 1; i <= len; i++) { //VIKTORZ --
        for (i = 1; i <= origlen; i++) { //VIKTORZ ++
            c[0] = w[offset + i - 1];
        ...
        if (k > 0) {
            ...
            //return new Hyphenation(new String(w, offset, len), res); //VIKTORZ --
            return new Hyphenation(new String(w, offset, origlen), res); //VIKTORZ ++
        } else {
            return null;
        }
    }
